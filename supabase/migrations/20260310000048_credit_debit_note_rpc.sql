-- Credit and debit notes reuse the invoice pipeline.
--
-- create_business_document already computed every amount in numeric(18,4) and
-- wrote the attachment, voucher, ledger lines, document and document lines in
-- one transaction. A note needs exactly that, with the party side reversed, so
-- the single `v_is_sale` direction flag becomes `v_party_is_debit`: the party is
-- debited on a sale and on a debit note, credited on a purchase and on a credit
-- note, and the trade, tax, TDS and round-off lines follow it.
--
-- Notes additionally carry the invoice they adjust and may not, in total,
-- exceed its value.
...
DECLARE
  ...
  v_is_sale boolean;
  v_party_is_debit boolean;
  v_is_note boolean;
  v_original_document_id uuid := NULLIF(p_payload ->> 'original_document_id', '')::uuid;
...
BEGIN
  ...
  END IF;
  IF v_document_type NOT IN ('sale', 'purchase', 'credit_note', 'debit_note') THEN
    RAISE EXCEPTION 'Document type must be sale, purchase, credit_note or debit_note';
  END IF;
  ...
  v_is_note := v_document_type IN ('credit_note', 'debit_note');
  -- A credit note mirrors a sale and a debit note mirrors a purchase, so the
  -- party side flips and every other line follows it.
  v_party_is_debit := v_document_type IN ('sale', 'debit_note');
  ...
  -- A note may not adjust away more than the invoice it references. The invoice
  -- is locked first so two concurrent notes cannot both pass this check.
  IF v_is_note AND v_original_document_id IS NOT NULL THEN
    SELECT * INTO v_original
    FROM public.business_documents
    WHERE id = v_original_document_id
    FOR UPDATE;
    IF NOT FOUND THEN
    ...
    IF v_total + COALESCE((
      ...
      FROM public.business_documents n
      WHERE n.original_document_id = v_original_document_id
    ), 0) > v_original.total_amount THEN
      RAISE EXCEPTION
        'Notes against invoice % would exceed its value of %',
        v_original.document_number, v_original.total_amount;
    END IF;
  END IF;
  ...
  INSERT INTO public.voucher_lines (
    ...
    v_party_id,
    CASE WHEN v_party_is_debit THEN v_total ELSE 0 END,
    CASE WHEN v_party_is_debit THEN 0 ELSE v_total END,
    v_document_number
  );
  v_line_no := 1;
  ...
  LOOP
    ...
    INSERT INTO public.voucher_lines (
      ...
      NULLIF(v_item ->> 'cost_centre_id', '')::uuid,
      NULLIF(v_item ->> 'salesman_id', '')::uuid,
      CASE WHEN v_party_is_debit THEN 0 ELSE (v_item ->> 'taxable_amount')::numeric END,
      CASE WHEN v_party_is_debit THEN (v_item ->> 'taxable_amount')::numeric ELSE 0 END,
      v_item ->> 'description',
      NULLIF(v_item ->> 'hsn_sac', ''),
  ...
  LOOP
    ...
    INSERT INTO public.voucher_lines (
      ...
      CASE WHEN v_party_is_debit THEN 0 ELSE (v_item ->> 'value')::numeric END,
      CASE WHEN v_party_is_debit THEN (v_item ->> 'value')::numeric ELSE 0 END,
      v_document_number
    );
  END LOOP;
  ...
  IF v_tds > 0 THEN
    ...
    INSERT INTO public.voucher_lines (
      ...
      CASE WHEN v_party_is_debit THEN v_tds ELSE 0 END,
      CASE WHEN v_party_is_debit THEN 0 ELSE v_tds END,
      'TDS ' || COALESCE(v_tds_section, '')
    );
  END IF;

  IF v_round_off <> 0 THEN
    v_round_debit := (v_party_is_debit AND v_round_off < 0) OR (NOT v_party_is_debit AND v_round_off > 0);
    v_line_no := v_line_no + 1;
    INSERT INTO public.voucher_lines (
      voucher_id, line_no, company_id, financial_year_id, ledger_id,
      debit_amount, credit_amount, narration
  ...
  INSERT INTO public.business_documents (
    ...
    eway_bill_no, created_by, original_document_id
  ) VALUES (
    ...
    v_eway_bill_no, auth.uid(), v_original_document_id
  )
...
COMMENT ON FUNCTION public.create_business_document(jsonb) IS
  'Atomically creates a sale, purchase, credit note or debit note: attachment, voucher, ledger lines, document and document lines. All money is numeric(18,4).';
