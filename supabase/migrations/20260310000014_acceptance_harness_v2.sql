-- Acceptance harness v2 (checks 1-13). Must DROP before recreate: return type gained user_maker vs migration 012.
DROP FUNCTION IF EXISTS public.seed_acceptance_fixture();
DROP FUNCTION IF EXISTS public.run_full_acceptance();
...
DECLARE
  ...
  v_maker uuid := 'dddddddd-dddd-dddd-dddd-dddddddddddd'::uuid;
  v_role_admin uuid;
  v_role_cashier uuid;
  v_role_maker uuid;
...
BEGIN
  ...
  BEGIN
    ...
    VALUES
      (v_admin, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'admin@acceptance.local', extensions.crypt('test-pass', extensions.gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Acc Admin"}'::jsonb, now(), now()),
      (v_user_b, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'userb@acceptance.local', extensions.crypt('test-pass', extensions.gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"User B"}'::jsonb, now(), now()),
      (v_cashier, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'cashier@acceptance.local', extensions.crypt('test-pass', extensions.gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Cashier L1"}'::jsonb, now(), now()),
      (v_maker, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'maker@acceptance.local', extensions.crypt('test-pass', extensions.gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Maker DE"}'::jsonb, now(), now())
  ...
  VALUES
    ...
    (v_maker, 'Maker DE', 'maker@acceptance.local', true)
  ...
  SELECT id INTO v_role_maker FROM public.roles WHERE code = 'approver';

  INSERT INTO public.user_roles (user_id, role_id) VALUES
    (v_admin, v_role_admin),
    (v_cashier, v_role_cashier),
    (v_maker, v_role_maker)
  ...
  VALUES
    ...
    (v_maker, v_company_a, true, true, true, false)
...
BEGIN
  ...
  -- 2) consecutive numbers (maker creates → admin approves → admin posts)
  PERFORM public.test_as_user(f.user_maker);
  INSERT INTO public.vouchers (
    company_id, financial_year_id, voucher_type_id, voucher_date, draft_ref, status, created_by
  ) VALUES (
    f.company_a, f.fy_a, f.ob_type_a, f.open_date,
    'DRAFT-P1-' || substr(gen_random_uuid()::text, 1, 8), 'draft', f.user_maker
  ...
  INSERT INTO public.vouchers (
    ...
    f.company_a, f.fy_a, f.ob_type_a, f.open_date,
    'DRAFT-P2-' || substr(gen_random_uuid()::text, 1, 8), 'draft', f.user_maker
  ...
  PERFORM public.test_as_user(f.user_admin);
  PERFORM public.approve_voucher(v_posted1);
  PERFORM public.approve_voucher(v_posted2);
  ...
  BEGIN
    ...
    check_no := 5; title := 'Posted voucher UPDATE rejects'; passed := v_cnt = 0; detail := format('affected=%s', v_cnt); RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    GET DIAGNOSTICS v_cnt = ROW_COUNT;
    check_no := 5; title := 'Posted voucher DELETE rejects'; passed := v_cnt = 0; detail := format('affected=%s', v_cnt); RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    GET DIAGNOSTICS v_cnt = ROW_COUNT;
    check_no := 5; title := 'Posted lines UPDATE rejects'; passed := v_cnt = 0; detail := format('affected=%s', v_cnt); RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    GET DIAGNOSTICS v_cnt = ROW_COUNT;
    check_no := 5; title := 'Postings UPDATE rejects'; passed := v_cnt = 0; detail := format('affected=%s', v_cnt); RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    GET DIAGNOSTICS v_cnt = ROW_COUNT;
    check_no := 5; title := 'Reopen posted via status rejects'; passed := v_cnt = 0; detail := format('affected=%s', v_cnt); RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
  ...
  -- -----------------------------------------------------------------------
  -- 10) Direct post of draft rejected
  -- -----------------------------------------------------------------------
  PERFORM public.test_as_user(f.user_maker);
  BEGIN
    EXECUTE 'SET LOCAL ROLE authenticated';
  EXCEPTION WHEN OTHERS THEN
    check_no := 10; title := 'SET ROLE authenticated (check 10)'; passed := false;
    detail := SQLERRM; RETURN NEXT; RETURN;
  END;
  ...
  INSERT INTO public.vouchers (
    ...
    f.company_a, f.fy_a, f.ob_type_a, f.open_date,
    'DRAFT-DIR-' || substr(gen_random_uuid()::text, 1, 8), 'draft', f.user_maker
  ) RETURNING id INTO v_draft;

  INSERT INTO public.voucher_lines (
    voucher_id, line_no, company_id, financial_year_id, ledger_id, debit_amount, credit_amount
  ) VALUES
    (v_draft, 1, f.company_a, f.fy_a, f.ledger_cash_a, 20, 0),
    (v_draft, 2, f.company_a, f.fy_a, f.ledger_offset_a, 0, 20);

  PERFORM public.test_as_user(f.user_admin);
  BEGIN
    EXECUTE 'SET LOCAL ROLE authenticated';
  EXCEPTION WHEN OTHERS THEN
    check_no := 10; title := 'SET ROLE authenticated (check 10 admin)'; passed := false;
    detail := SQLERRM; RETURN NEXT; RETURN;
  END;

  BEGIN
    PERFORM public.post_voucher(v_draft);
    check_no := 10; title := 'Direct post of draft rejected'; passed := false; detail := 'draft post succeeded'; RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
    check_no := 10; title := 'Direct post of draft rejected';
    passed := (SQLERRM ILIKE '%approved%');
    detail := SQLERRM;
    RETURN NEXT;
  END;

  -- -----------------------------------------------------------------------
  -- 11) Maker cannot approve own voucher by default
  -- -----------------------------------------------------------------------
  PERFORM public.test_as_user(f.user_maker);
  BEGIN
    EXECUTE 'SET LOCAL ROLE authenticated';
  EXCEPTION WHEN OTHERS THEN
    check_no := 11; title := 'SET ROLE authenticated (check 11)'; passed := false;
    detail := SQLERRM; RETURN NEXT; RETURN;
  END;

  BEGIN
    PERFORM public.approve_voucher(v_draft);
    check_no := 11; title := 'Maker cannot approve own voucher'; passed := false; detail := 'self-approve ok'; RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
    check_no := 11; title := 'Maker cannot approve own voucher';
    passed := (SQLERRM ILIKE '%Maker cannot approve%' OR SQLERRM ILIKE '%own voucher%');
    detail := SQLERRM;
    RETURN NEXT;
  END;

  -- Admin (not maker) can approve the same voucher
  PERFORM public.test_as_user(f.user_admin);
  BEGIN
    EXECUTE 'SET LOCAL ROLE authenticated';
  EXCEPTION WHEN OTHERS THEN
    check_no := 11; title := 'SET ROLE authenticated (check 11 admin)'; passed := false;
    detail := SQLERRM; RETURN NEXT; RETURN;
  END;

  BEGIN
    PERFORM public.approve_voucher(v_draft);
    check_no := 11; title := 'Non-maker admin can approve'; passed := true; detail := 'ok'; RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
    check_no := 11; title := 'Non-maker admin can approve'; passed := false; detail := SQLERRM; RETURN NEXT;
  END;

  -- -----------------------------------------------------------------------
  -- 12) Unauthorised ensure_opening_balance_offset_ledger rejected
  -- -----------------------------------------------------------------------
  PERFORM public.test_as_user(f.user_b);
  BEGIN
    EXECUTE 'SET LOCAL ROLE authenticated';
  EXCEPTION WHEN OTHERS THEN
    check_no := 12; title := 'SET ROLE authenticated (check 12)'; passed := false;
    detail := SQLERRM; RETURN NEXT; RETURN;
  END;
  ...
  IF EXISTS (
    ...
    check_no := 12; title := 'SET ROLE authenticated (check 12)'; passed := false;
    detail := format('current_user=%s bypasses RLS', CURRENT_USER);
    RETURN NEXT; RETURN;
  END IF;

  BEGIN
    PERFORM public.ensure_opening_balance_offset_ledger(f.company_a);
    check_no := 12; title := 'Unauthorised OB-OFFSET RPC rejects'; passed := false; detail := 'rpc ok'; RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
    check_no := 12; title := 'Unauthorised OB-OFFSET RPC rejects'; passed := true; detail := SQLERRM; RETURN NEXT;
  END;

  -- -----------------------------------------------------------------------
  -- 13) Unauthorised seed/setup helpers rejected
  -- -----------------------------------------------------------------------
  -- still under user_b + SET ROLE authenticated from check 12
  BEGIN
    PERFORM public.seed_company_voucher_types(f.company_a);
    check_no := 13; title := 'Unauthorised seed_company_voucher_types rejects'; passed := false; detail := 'seed ok'; RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
    check_no := 13; title := 'Unauthorised seed_company_voucher_types rejects'; passed := true; detail := SQLERRM; RETURN NEXT;
  END;

  BEGIN
    PERFORM public.create_monthly_periods(f.fy_a);
    check_no := 13; title := 'Unauthorised create_monthly_periods rejects'; passed := false; detail := 'periods ok'; RETURN NEXT;
  EXCEPTION WHEN OTHERS THEN
    check_no := 13; title := 'Unauthorised create_monthly_periods rejects'; passed := true; detail := SQLERRM; RETURN NEXT;
  END;
END;
$$;
...
COMMENT ON FUNCTION public.run_full_acceptance() IS
  'Acceptance 1–9 plus 10 direct-draft-post, 11 maker self-approve, 12 OB-OFFSET authz, 13 seed helper authz.';
