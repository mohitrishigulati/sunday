...
export default async function BusinessPage() {
  ...
  const [
    ...
    // Reversed allocations no longer settle anything, so they must not count
    // toward allocated totals or on-account balances.
  ...
  return <div className="space-y-8">
    <PageHeader title="Sales, purchase & bill settlement" description="Invoice-line GST/TDS accounting, bill-wise outstanding allocation and due-date ageing." />
    <BusinessDocumentForm companies={companies ?? []} years={years ?? []} parties={parties ?? []} ledgers={ledgers ?? []} costCentres={costCentres ?? []} salesmen={salesmen ?? []} />
    <DataTable columns={["Company","Type","Invoice","Date","Party","Taxable","GST","TDS","Total","Outstanding","Ageing","Status","Attachment","Actions","Allocate receipt/payment"]} rows={(documents ?? []).map((document) => {
